...
- Manual trigger via `ropCheckCycles` opcode
...
- Added 6 new opcodes:
  - `ropNewRef` - Allocate heap object
  - `ropIncRef` - Increment reference count
  - `ropDecRef` - Decrement reference count
  - `ropNewWeak` - Create weak reference
  - `ropWeakToStrong` - Promote weak to strong
  - `ropCheckCycles` - Manual cycle detection
...
1. **Change `ekNew` compilation** (regvm_compiler.nim:433)
   ...
   - Needed: Emit `ropNewRef` to allocate on heap

2. **Field access with heap objects**
   - `ropGetField` / `ropSetField` need to handle `vkRef` values
...
3. **Reference counting emission**
   - Emit `ropIncRef` on reference assignments
   - Emit `ropDecRef` at end of scope (use existing defer mechanism)
   - Leverage existing lifetime analysis data

4. **Object literal initialization**
   - After `ropNewRef`, set fields on heap object
