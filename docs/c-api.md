...
| `array[T]` | `EtchValue*` elements | `etch_value_new_array()` | `etch_value_array_*()` |
...
- Build the context with `debug=true` (or call `etch_context_set_debug(ctx, 1)`).
- Set `ETCH_DEBUG_PORT=<port>` (and optionally `ETCH_DEBUG_TIMEOUT=<ms>`) before compiling.
- As soon as you compile a script, Etch opens the TCP port and keeps listening for VSCode.
- The same debug session covers both `etch_execute` and repeated `etch_call_function` invocations,
  so you can attach to long-running hosts (see `demo/main.cpp`) without restarting.
- Disconnections only close the client socket – the server keeps running, so you can reattach later.
- The VM continuously polls for debugger commands even while Etch code is running, so you can attach, pause, or add breakpoints at any moment (they take effect immediately in the next instruction).
