...
proc dumpLifetimes*(tracker: LifetimeTracker) =
  ...
  stderr.writeLine "=== Variable Lifetimes ==="
  for name, ranges in tracker.variableMap:
    for r in ranges:
      stderr.writeLine "  ", name, ": R", r.register,
           " [", r.startPC, "..", r.endPC, "]",
           " def@", r.defPC,
           " lastUse@", r.lastUsePC,
           " scope=", r.scopeLevel

  stderr.writeLine "=== Destructor Points ==="
  var sortedPCs: seq[int] = @[]
  for pc in tracker.destructorPoints.keys:
    sortedPCs.add(pc)
  sortedPCs.sort()
  for pc in sortedPCs:
    stderr.writeLine "  PC ", pc, ": ", tracker.destructorPoints[pc].join(", ")
