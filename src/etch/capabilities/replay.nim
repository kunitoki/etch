...
proc newReplayEngine*(vm: VirtualMachine, snapshotInterval: int = DEFAULT_SNAPSHOT_INTERVAL): ReplayEngine =
  result = ReplayEngine(
    snapshots: @[],
...
proc copyRegisterFrame(frame: RegisterFrame): RegisterFrame =
  ...
  when not defined(deploy):
    result.funcName = frame.funcName
...
proc takeSnapshot*(engine: ReplayEngine, instrIdx: int) =
  ...
  var sourceLine = 0
  if instrIdx >= 0:
    let debug = vm.program.getDebugInfo(instrIdx)
    sourceFile = debug.sourceFile
    sourceLine = debug.line
...
proc seekTo*(engine: ReplayEngine, targetStatement: int) =
  if engine.snapshots.len == 0:
    return

  # Clamp target to valid range
  let target = max(0, min(targetStatement, engine.totalStatements))
...
proc truncateAndRestart*(engine: ReplayEngine) =
  ...
  for delta in engine.deltas:
    if delta.instructionIndex <= currentStatement:
      newDeltas.add(delta)
  ...
  engine.takeSnapshot(currentStatement)
    ...
    for _ in 0..<snapshotCount:
      ...
      for _ in 0..<frameCount:
        ...
        for i in 0..<regCount:
          ...
          of vkEnum:
            let typeId = stream.readInt32()
            let intVal = stream.readInt64()
            let slen = int(stream.readUint32())
            var str = ""
            if slen > 0:
              str.setLen(slen)
              discard stream.readData(addr str[0], slen)
            frame.regs[i] = makeEnum(typeId, intVal, str)
          else:
      ...
      for _ in 0..<globalCount:
        ...
        of vkEnum:
          ...
          if slen > 0:
            str.setLen(slen)
            discard stream.readData(addr str[0], slen)
          globals[key] = makeEnum(typeId, intVal, str)
        else:
          globals[key] = makeNil()
...
proc restoreReplayEngine*(vm: VirtualMachine, totalStatements: int, snapshotInterval: int,
