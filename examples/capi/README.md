...
Need to run without C++ exceptions? Define `ETCH_CPP_EXCEPTIONS 0` before including the header so
fatal errors print to stderr and call `std::abort()` instead of throwing:
...
- Template-based host registration with automatic argument conversions
- Variadic `callFunction` accepting native C++ types (ints, strings, spans, etc.)

```cpp
etch::Context ctx;
ctx.registerFunction("add", [](int64_t a, int64_t b) {
    return a + b;  // returns int -> auto wrapped into etch::Value
});

int64_t sum = ctx.callFunction("add", 21, 21).toInt();
```
...
| `array[T]` | `EtchValue*` | `etch_value_new_array()` | `etch_value_array_*()` |
| `option[T]` | `EtchValue` | `etch_value_new_some()` / `_none()` | `etch_value_option_unwrap()` |
| `result[T, E]` | `EtchValue` | `etch_value_new_ok()` / `_err()` | `etch_value_result_unwrap_*()` |

Need to duplicate a handle? `etch_value_clone()` performs a deep copy so the caller owns a fresh
value without mutating the original.

### Arrays, Option, and Result Helpers

Use `etch_value_new_array()` plus the `etch_value_array_*()` helpers to construct and
mutate Etch arrays directly from C. `etch_value_new_some()` / `_none()` and
`etch_value_new_ok()` / `_err()` make it easy to work with `option`/`result` values, and
the corresponding unwrap helpers return fresh handles that you own.
